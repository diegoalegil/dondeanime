...
> Archivo de contexto para Codex. Se carga automáticamente al ejecutar `Codex` dentro de este repo. Mantener actualizado conforme avanza el proyecto.
...
Claves: `POSTGRES_PASSWORD` (autogenerada), `TMDB_API_KEY` (la misma que en .env local), `VERCEL_DEPLOY_HOOK` (URL del Deploy Hook configurado en Vercel), `SCHEDULING_ENABLED=true`, `TELEGRAM_ENABLED=false` por defecto, `TELEGRAM_BOT_TOKEN` y `TELEGRAM_CHAT_ID` solo si se activan alertas críticas.
...
### Cómo coordina Diego con Codex (y con Codex)
...
- **Codex** (esta sesión o nuevas) interviene a petición de Diego para verificar bloques de Codex (`"verifica el bloque X"`) o resolver dudas estratégicas que excedan la autonomía de Codex.
