...
Premium con Stripe usa `STRIPE_SECRET_KEY`, `PUBLIC_STRIPE_PUBLISHABLE_KEY`, `STRIPE_PRICE_ID`, `STRIPE_WEBHOOK_SECRET`, `STRIPE_SUCCESS_URL` y `STRIPE_CANCEL_URL`. Durante implementación se usan claves test (`sk_test_...` y `pk_test_...`); Diego cambia a live cuando active pagos reales.
