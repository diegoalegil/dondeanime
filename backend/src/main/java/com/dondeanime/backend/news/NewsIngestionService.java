 ...
 * {@code source_url}: una noticia ya ingerida se salta.
 */
@Service
public class NewsIngestionService {
    ...
    private NewsIngestionResult.SourceResult ingestSource(NewsSource source) {
        ...
        for (FetchedNewsItem item : fetched) {
            String url = item.url();
            if (url.length() > MAX_URL || !seenInBatch.add(url) || itemRepository.existsBySourceUrl(url)) {
                skipped++;
                continue;
            }
            switch (persist(source, item)) {
    ...
    private PersistOutcome persist(NewsSource source, FetchedNewsItem fetched) {
        try {
